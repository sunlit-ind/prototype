...
(function (jQuery) {
	jQuery.fn.ganttView = function (options) {
		...
		els.each(function () {
			var container = jQuery(this); //要素の取得
	...
	var Chart = {
			...
			addBlockContainers: function (div) {	//データを配列に詰める
				var opts = Chart.opts;
				var data = opts.data;
				var showDayOfWeek = opts.showDayOfWeek;
			...
			setBlockContainersWidth: function(div) {	//チャートブロック描画のデータセット
				var opts = Chart.opts;
				var data = opts.data;
			...
			addBlocks: function (div) {	//チャートブロック描画
				var opts = Chart.opts;
				var data = opts.data;
			...
			setCount: function (obj, start, end) {	//データのカウント
				var opts = Chart.opts;
				var weekends = 0;
			...
			createGrid: function(months) {	//カレンダーグリッド生成
				var opts = Chart.opts;
				var data = opts.data;
				var cellWidth = opts.cellWidth;
				var cellHeight = opts.cellHeight;
			...
			createBlock: function(itemIdx, series, rows, rowIdx, opts) { //ブロックの生成
				var data = opts.data;
			...
			addSeries: function(d, s, itemIdx, mergedPoint, newPoint) {		//Series
	...
	jQuery.extend(jQuery.fn.ganttView, {	//オブジェクトのマージ
		addData: function(d) {
			var data = Chart.opts.data;
